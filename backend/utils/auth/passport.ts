...
passport.use(new LocalStrategy(
  async ( username:string, password:string, done: CallableFunction ) => {                               //Get username and password from authetification client form
    ...
    if(!client)
      ...
      if(!match)  return done(null, false, { message: 'Password not correct' });    //IF password doesn't  correct, then show error message
      else        return done(null, client, { message: `Welcome ${ String(client.fullname).toUpperCase() }` });   //if is correct, then show ok message
    }
  }
));
/**
